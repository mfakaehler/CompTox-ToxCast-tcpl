...
tcplSendQuery <- function(query, db = getOption("TCPL_DB"),
  ...
  if (getOption("TCPL_DRVR") == "PostgreSQL") {
    ...
    db_pars <- list(drv = dbDriver("PostgreSQL"),
                    user = getOption("TCPL_USER"),
                    password = getOption("TCPL_PASS"),
                    host = getOption("TCPL_HOST"),
                    dbname = db)
    # note that double-quotes in PostgreSQL have a different meaning then in MySQL
    query <- gsub("\\\"", "'", query)
    # note that PostgreSQL doesn't like backticks
    query <- gsub("`", "", query)
